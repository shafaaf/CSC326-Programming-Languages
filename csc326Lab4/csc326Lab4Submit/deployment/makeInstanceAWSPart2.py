...
print "Now do the trasfer and run it"

#subprocess
#ssh -i key_pair.pem ubuntu@<PUBLIC-IP-ADDRESS>
#scp -i key_pair.pem <FILE-PATH> ubuntu@<PUBLIC-IP-ADDRESS>:~/<REMOTE-PATH>
print "Moving the setupAWS.sh to AWS Ubuntu instance at ip address: ", myPublicIp
...
print "CSC326 Search engine will be running on %s:80" % myPublicIp
print "Instance ID and public IP have returned.\n\n\n\n\n\n\n\n\n\n"

with open("deploymentResult.txt", "w") as text_file:
	text_file.write("Instance ID and Instance IP address: (%s, %s)" % (myInstanceId, myPublicIp))
